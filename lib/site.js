...
import SiteEmbed from './site.embed';
...
class Site {
	...
	/**
	 * Create a `SiteEmbed` instance
	 *
	 * @param {String} url - embed url
	 * @return {SiteEmbed} SiteEmbed instance
	 */
	embed( url ) {
		return new SiteEmbed( url, this._id, this.wpcom );
	}
	 ...
	 * Shorthand of wpcom.site( siteId ).embed( url ).render( query, fn );
	 *
	 * Note: The current user must have publishing access.
	 *
	 * @param {String} url - embed url
	...
	renderEmbed( url, query, fn ) {
		return this.embed( url ).render( fn );
	}
